...
<template>
  <v-app id="inspire">
    ...
    <v-app-bar app>
      ...
      <v-btn icon color="error" class="mr-12" @click.prevent="logOut">
    ...
    <v-navigation-drawer v-model="drawer" position="fixed" temporary>
      ...
      <v-card class="mx-auto" width="300">
        <v-list nav>
          ...
          <v-list-item v-for="([title, icon, ruta], index) in homeItems" :key="index" :to="{ name: ruta }" exact>
            <v-list-item-icon>
              <v-icon>{{ icon }}</v-icon>
            </v-list-item-icon>
          ...
          <v-list-group :value="true" prepend-icon="mdi-account-circle">
            ...
            <!--   -->
            <v-list-item v-for="([title, icon, ruta], index) in adminItems" :key="index" :to="{ name: ruta }" link>
              <v-list-item-icon>
                <v-icon>{{ icon }}</v-icon>
              </v-list-item-icon>
              <v-list-item-title>
          ...
          <v-list-group v-if="userStore.user.rol === 'Administrador'" prepend-icon="mdi-account-circle" :value="true">
            <template v-slot:activator>
              <v-list-item-content>
                <v-list-item-title>
                  Permisos
            ...
            <v-list-item v-for="([title, icon, ruta], index) in permisosItems" :key="index" :to="{ name: ruta }" link>
              <v-list-item-icon>
                <v-icon>{{ icon }}</v-icon>
              </v-list-item-icon>
              <v-list-item-title>
    ...
    <v-main tag="dashboard" class="grey lighten-2">
      <v-container>
        <router-view />
      </v-container>
    </v-main>
